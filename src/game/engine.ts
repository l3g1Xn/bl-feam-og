// @ts-nocheck
/* recovered engine with leet enemy names */
...
function drawCards(p, n) {
	...
	const drawn = [];
	let heroHp = p.heroHp;
	for (let i = 0; i < n; i++) if (deck.length === 0) {
		fatigue += 1;
		fatigueDmg += fatigue;
		heroHp -= fatigue;
	...
	return {
		player: {
			...
			hand,
			fatigue,
			heroHp: Number.isFinite(heroHp) ? heroHp : 0
		},
		drawn,
		fatigueDmg
	};
}
function createMinion(def) {
	...
	return {
		uid: nextUid(),
		defId: def.id,
		attack: Math.max(0, def.attack ?? 0),
		health: Math.max(1, def.health ?? 1),
		maxHealth: Math.max(1, def.health ?? 1),
		keywords,
...
export function createNewGame(difficulty: "normal" | "hard" = "normal", playerDeck?: string[]): any {
	const deck = playerDeck && playerDeck.length >= 20 ? playerDeck : buildStarterDeck();
	...
	return {
		...
		log: [
			...
			{ id: 2, text: "Opening hand — keep a curve you can afford.", tone: "system" },
		],
		logSeq: 2,
		lastPreview: null,
		hoverPreview: null,
...
export function confirmMulligan(state, redrawIndices) {
	if (state.phase !== "mulligan") return state;
	let player = state.player;
	const keep = [];
	const redraw = [];
	player.hand.forEach((id, i) => {
		if (redrawIndices.includes(i)) redraw.push(id);
		else keep.push(id);
	});
...
export function selectHandCard(state, index) {
	...
	if (state.player.board.length >= 7) return {
		...state,
		message: "Board is full (7)."
	};
	return playMinion(state, index);
}
function playMinion(state, handIndex) {
	...
	next = pushLog(next, `Play ${def.name} ${stats}${kw} (−${def.cost} mana → ${next.player.mana} left)`, "player");
	next = pushLog(next, `Value: (${minion.attack}+${minion.health})/${def.cost} = ${((minion.attack + minion.health) / Math.max(1, def.cost)).toFixed(2)} stats/mana`, "math");
	return next;
}
export function resolveSpellTarget(state, target) {
	...
	if (!isValidSpellTarget(def.spell, target, state)) return {
		...state,
		message: "Invalid target for this spell."
	};
	return playSpell(state, handIndex, target);
}
export function playSpell(state, handIndex, target) {
	const cardId = state.player.hand[handIndex];
	if (!cardId) return state;
	const def = getCard(cardId);
	...
	if (spellNeedsTarget(def.spell)) {
		if (!target || !isValidSpellTarget(def.spell, target, state)) return {
			...state,
			message: "Invalid target for this spell."
		};
	}
	...
	next = pushLog(next, `Cast ${def.name} (−${def.cost} mana → ${next.player.mana} left)`, "player");
	next = applySpell(next, "player", def.spell, target);
	return checkWinner(next);
}
function applySpell(state, caster, spell, target) {
	let next = state;
	const ally = caster === "player" ? "player" : "enemy";
	const foe = caster === "player" ? "enemy" : "player";
	const sp = Math.max(0, Math.floor(next[ally].spellPower || 0));
	switch (spell.kind) {
		case "draw": {
			const p = next[ally];
			const d = drawCards(p, spell.count);
			next = {
				...next,
				[ally]: d.player
			};
			next = pushLog(next, `Draw ${spell.count} · hand ${d.player.hand.length}`, "math");
			break;
		}
		case "spell_power": {
			const p = next[ally];
			...
			next = pushLog(next, `Spell Power +${spell.amount} → ${spellPower}`, "math");
		...
		case "buff_all_friendly": {
			...
			next = pushLog(next, `All friendlies +${spell.attack}/+${spell.health}`, "math");
			break;
		}
		case "aegis":
			if (target?.kind === "minion" && target.side === ally) {
				...
				next = pushLog(next, `Aegis — Immune this turn` + (spell.shield ? " + Shield" : ""), "math");
			}
			break;
		case "dominus_reximus":
			if (target?.kind === "minion" && target.side === ally) {
				...
				const name = getCard(board.find((m) => m.uid === target.uid)?.defId ?? "squire").name;
				next = {
					...next,
					[ally]: {
						...next[ally],
				...
				next = pushLog(next, `Dominus Reximus on ${name}: +${spell.targetAttack}/+${spell.targetHealth} Immune Reborn · others +${spell.othersAttack}/+${spell.othersHealth} Immune this turn`, "math");
			}
			break;
		case "heal":
			if (spell.target === "friendly_hero" || target?.kind === "hero" && target.side === ally) {
				const p = next[ally];
				const before = p.heroHp;
				const heroHp = Math.min(p.heroMaxHp, Math.max(0, p.heroHp + spell.amount));
				next = {
					...next,
					[ally]: {
						...p,
				...
				next = pushLog(next, `Heal ${before} → ${heroHp} (+${heroHp - before})`, "math");
		...
		case "buff":
			if (target?.kind === "minion" && target.side === ally) {
				...
				next = pushLog(next, `Buff +${spell.attack}/+${spell.health} on ${name}`, "math");
			}
			break;
		case "damage": {
			const amt = spell.amount + sp;
			if (spell.target === "all_enemy_minions") {
				for (const m of [...next[foe].board]) next = damageMinion(next, foe, m.uid, amt);
				next = pushLog(next, `AoE ${amt} to all enemy minions (SP ${sp})`, "math");
				...
				next = {
					...
					[foe]: {
						...p,
						heroHp: Math.max(0, p.heroHp - amt)
					}
				};
				for (const m of [...next[foe].board]) next = damageMinion(next, foe, m.uid, amt);
				next = pushLog(next, `AoE ${amt} to all enemies (SP ${sp})`, "math");
			} else if (target) if (target.kind === "hero") {
				...
				next = pushLog(next, `${amt} dmg to ${side === "player" ? "you" : "enemy"} → ${next[side].heroHp} HP (SP ${sp})`, "math");
			} else {
		...
		case "damage_and_draw": {
			...
			} else next = damageMinion(next, target.side, target.uid, amt);
			if (spell.draw > 0) {
				const d = drawCards(next[ally], spell.draw);
				next = {
					...next,
			...
			next = pushLog(next, `${amt} dmg (SP ${sp})` + (spell.draw > 0 ? ` + draw ${spell.draw}` : ""), "math");
			break;
		}
		case "damage_heal": {
			const amt = spell.damage + sp;
			...
			next = pushLog(next, `${amt} dmg (SP ${sp}) · heal ${spell.heal} → ${heroHp} HP`, "math");
			break;
		}
	}
...
function damageMinion(state, side, uid, amount) {
	...
	const board = state[side].board.map((m) => {
		...
		if (amount < 0) {
			...
			return {
				...m,
				health: Number.isFinite(health) ? health : m.health
			};
		}
		if (amount > 0 && isImmune(m)) return m;
		if (m.shield && amount > 0) return {
			...m,
			shield: false
		};
...
function resolveBoard(state) {
	const revive = (board) => {
		...
		for (const m of board) {
			...
			if (m.keywords.includes("reborn")) out.push({
				...m,
				health: 1,
	...
	return {
		...
		player: {
			...
			spellPower: Number.isFinite(state.player.spellPower) ? state.player.spellPower : 0
		},
		enemy: {
			...state.enemy,
			board: revive(state.enemy.board),
			heroHp: Number.isFinite(state.enemy.heroHp) ? state.enemy.heroHp : 0,
			spellPower: Number.isFinite(state.enemy.spellPower) ? state.enemy.spellPower : 0
		}
	};
}
export function performAttack(state, attackerUid, target) {
	const atkSide = state.player.board.some((m) => m.uid === attackerUid) ? "player" : "enemy";
	...
	if (target.kind === "hero") {
		...
		const preview = computeCombatPreview(attacker, "hero", defSide === "player" ? "You" : (state.enemyName || "Enemy"), hero.heroHp, false);
		...
		if (preview.lifestealHeal > 0) atkPlayer = {
			...atkPlayer,
			heroHp: Math.min(atkPlayer.heroMaxHp, atkPlayer.heroHp + preview.lifestealHeal)
		};
		next = {
			...next,
			[defSide]: {
		...
		next = pushLog(next, `${preview.attackerName} hits ${preview.defenderName} for ${preview.damageToDefender}`, atkSide === "player" ? "player" : "enemy");
	} else {
		const defender = next[defSide].board.find((m) => m.uid === target.uid);
		if (!defender) return state;
		const preview = computeCombatPreview(attacker, defender, getCard(defender.defId).name, defender.health, defender.shield);
		const atkBoard = next[atkSide].board.map((m) => {
			if (m.uid !== attackerUid) return m;
			let shield = m.shield;
			let health = m.health;
			if (preview.damageToAttacker > 0) if (shield) shield = false;
			else health -= preview.damageToAttacker;
			return {
				...m,
				health: Number.isFinite(health) ? health : 0,
				shield,
		...
		const defBoard = next[defSide].board.map((m) => {
			...
			let shield = m.shield;
			let health = m.health;
			if (preview.shieldAbsorbed) shield = false;
			else health -= preview.damageToDefender;
			return {
				...m,
				health: Number.isFinite(health) ? health : 0,
				shield
		...
		next = pushLog(next, preview.formula, "math");
		next = pushLog(next, `${preview.attackerName} vs ${preview.defenderName}` + (preview.defenderDies ? " — destroyed" : "") + (preview.attackerDies ? " — attacker falls" : ""), atkSide === "player" ? "player" : "enemy");
		next = resolveBoard(next);
	}
	return checkWinner(next);
}
function checkWinner(state) {
	if (state.enemy.heroHp <= 0) return {
		...state,
		phase: "victory",
	...
	if (state.player.heroHp <= 0) return {
		...state,
		phase: "defeat",
...
export function endPlayerTurn(state) {
	...
	if (d.fatigueDmg > 0) next = pushLog(next, `Enemy fatigue ${d.fatigueDmg}`, "system");
	return checkWinner(next);
}
export function afterEnemyTurn(state) {
	...
	if (d.fatigueDmg > 0) next = pushLog(next, `Fatigue ${d.fatigueDmg}`, "system");
...
export function setHoverPreview(state, preview) {
	return {
		...state,
		hoverPreview: preview
	};
}
export function enemyPlayMinion(state, handIndex) {
	...
	next = pushLog(next, `Enemy plays ${def.name} ${minion.attack}/${minion.health} (−${def.cost})`, "enemy");
	return next;
}
export function enemyPlaySpell(state, handIndex, target) {
	const cardId = state.enemy.hand[handIndex];
	if (!cardId) return state;
	const def = getCard(cardId);
	if (def.type !== "spell" || !def.spell || def.cost > state.enemy.mana) return state;
	const hand = [...state.enemy.hand];
	hand.splice(handIndex, 1);
	let next = {
		...state,
...
export function enemyAttack(state, attackerUid, target) {
