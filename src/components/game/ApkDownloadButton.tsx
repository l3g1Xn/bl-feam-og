...
import { APK_DOWNLOAD_NAME, BUILD_ID } from "@/game/brand";
  ...
  const onClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        setProgress(`Saved ${APK_DOWNLOAD_NAME} (${mb} MB)`);
      }
    } catch (ex) {
  ...
  return (
    <div className={cn("flex flex-col gap-1.5", className)}>
      <button
        ...
        aria-label={`Download ${APK_DOWNLOAD_NAME}`}
        data-apk-download="1"
      >
        {busy ? (
      ...
      {!busy && !ok && !err && (
        <p className="text-center text-[0.6rem] text-fg-subtle">
          {APK_DOWNLOAD_NAME} · ~44 MB · build {BUILD_ID}
        </p>
      )}
    </div>
  );
